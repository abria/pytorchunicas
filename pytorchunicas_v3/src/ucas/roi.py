...
from ucas.utils import partition
...
class CvROI(data.Dataset):
    ...
    def __init__(self, roi_files, img_folder, img_prefix='', img_suffix='', img_channel=None, img_list='', train=False, crossvalid=(1, 1), class_weights=None, class_max_counts=None, class_min_counts=None, preprocessing=None, augmentation=None, verbose=True):
        """
        Args:
            ...
            with open(roi_file) as f:
                ...
                for line in f:
                    ...
                    if tokens[0] not in self.images_pool:
                        ...
                        if mcroi:
                            img_path = img_folder + '/' + tokens[0][0:8] + '/st' + tokens[0][8:10] + '/' + img_prefix + tokens[0] + img_suffix
                        else:
                            img_path = img_folder + '/' + img_prefix + tokens[0] + img_suffix
                        ...
                        if img_path.endswith('.cvmat'):
                            with open(img_path, "rb") as cvmat:
                                ...
                                if img_channels != 1:
                                    raise ValueError('Unsupported channels in .cvmat (only 1-channel .cvmat are supported)')
                                a = array.array('f')
                    ...
                    if len(tokens) is 3:
                        ...
                        raise ValueError('Expected 3 or 5 tokens at line ' + line.rstrip('\r\n') + ' but found ' + str(len(tokens)))
                    ...
                    if class_max_counts and class_max_counts[class_i] and self.class_counts[class_i] >= class_max_counts[class_i]:
                        break
        ...
        if class_weights and train:
            ...
            str(len(class_weights)) + ' class probabilities have been provided, but there are ' + str(len(self.class_counts)) + ' classes'
            
            maj_class = self.class_counts.tolist().index(max(self.class_counts))
            for class_i in range(self.n_classes):
                self.class_augmentations.append(int(round(self.class_counts[maj_class] * (class_weights[class_i] / class_weights[maj_class]) - self.class_counts[class_i])))
            ...
            str(len(class_min_counts)) + ' class min counts have been provided, but there are ' + str(len(self.class_counts)) + ' classes'
        ...
        for c in range(self.n_classes):
            self.class_total_counts[c] = self.class_counts[c] + (self.class_augmentations[c] if self.class_augmentations else 0)
